...
(test "some_function(shape, x, y + 1, z);\n"
    (fmt #f (c-expr '(some_function shape x (+ y 1) z))))
...
"
    (fmt #f (c-expr '(if (&& (< y 255) (== (pred shape x y z) 0)) 2 3))))

(test "heightmap[i + k * 16];\n"
    (fmt #f (c-expr '(vector-ref heightmap (+ i (* k 16))))))
...
(test "x == 0 && (y == 2 || y == 3);\n"
    (fmt #f (c-expr '(%and (== x 0) (%or (== y 2) (== y 3))))))
...
(test "#define foo(x, y) (2 * ((x) + (y) + z))\n"
    (fmt #f (cpp-define '(foo (int x) (int y)) '(* 2 (+ x y z)))))
